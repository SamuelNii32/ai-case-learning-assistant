...
export default function Header() {
  ...
  if (import.meta.env.DEV) {
    try {
      console.debug('[Header] auth', { loggedIn: auth?.loggedIn, token: auth?.token, ls: typeof window !== 'undefined' ? localStorage.getItem('authToken') : null })
    } catch {
      /* ignore */
    }
  }
          ...
          {auth?.loggedIn ? (
            <div className="flex items-center gap-3 relative" ref={userMenuRef}>
              ...
              >
                ...
                <svg className="w-3 h-3 text-slate-400" viewBox="0 0 20 20" fill="currentColor" aria-hidden>
            ...
            <div className="pt-2">
              {auth?.loggedIn ? (
                <div className="space-y-2">
                  <div className="px-2">
                    <div className="text-sm text-slate-700 mb-2">{auth.user?.fullName || auth.user?.email}</div>
                  </div>
